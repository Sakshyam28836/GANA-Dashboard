...
class User:
        ...
        if method == "updateSelectedBot":
            ...
            try:
                ...
                guilds = {
                    ...
                    for guild in resp if (int(guild.get('permissions', 0)) & 8) == 8 or (int(guild.get('permissions', 0)) & 32) == 32
                }
    ...
    async def send(self, payload: Dict) -> None:
        if self._websocket:
            LOGGER.info(f"[USER OUT] {payload}")
            await self._websocket.send_json(payload)
            
    async def _listen(self) -> None:
        while True:
            if not self.bot:
                LOGGER.info(f"Broadcasting initBot for user {self.id}"); await BotPool.broadcast({"op": "initBot", "userId": self.id})

            data = await self._websocket.receive()
            LOGGER.info(f"[USER IN] {data}")
...
class Bot:
    ...
    async def send(self, payload: Dict) -> None:
        if self.is_connected:
            ...
            LOGGER.info(f"[IPC OUT] {payload}")
            await self._websocket.send_json(payload)
    
    async def _listen(self):
        while True:
            data = await self._websocket.receive()
            LOGGER.info(f"[IPC IN] {data}")
            ...
            if (guild_id := data.get("guildId")):
                ...
                if method == "updateGuild":
                    ...
                    if user:
                        await guild.add_user(user, init_player=False) if data.get("isJoined") else await guild.remove_user(user)
            ...
            if user_id := data.get("userId"):
                ...
                if user:
                    if method == "initUser":
                        try:
                            from utils import requests_api, DISCORD_API_BASE_URL
                            resp = await requests_api(f'{DISCORD_API_BASE_URL}/users/@me/guilds', headers={'Authorization': f'Bearer {user.access_token}'})
                            bot_guilds = data.get("botGuilds", [])
                            if resp:
                                admin_guilds = []
                                for guild in resp:
                                    perms = int(guild.get('permissions', 0))
                                    # Administrator is 8, Manage Server is 32
                                    if (perms & 8) == 8 or (perms & 32) == 32:
                                        # Only include if bot is also in the guild (from botGuilds)
                                        if guild['id'] in bot_guilds:
                                            admin_guilds.append({
                                                "id": guild['id'],
                                                "name": guild['name'],
                                                "icon": f"https://cdn.discordapp.com/icons/{guild['id']}/{guild['icon']}.webp" if guild.get('icon') else None
                                            })
                                data["adminGuilds"] = admin_guilds
                        except Exception as e:
                            import traceback
                            LOGGER.error(f"INITUSER ERROR: {e}")
...
class BotPool:
    ...
    async def broadcast(cls, data: Dict) -> None:
        LOGGER.info(f"BotPool.broadcast: {data} to {len(cls._bots)} bots")
