...
const methods = {
    ...
    initUser: function (player, data) {
        ...
        player.adminGuilds = data.adminGuilds || [];
        if (player.adminGuilds.length > 0) {
            let gridHtml = "";
            player.adminGuilds.forEach(g => {
                let iconUrl = g.icon || "https://cdn.discordapp.com/embed/avatars/0.png";
                gridHtml += `
                <div class="card admin-server-card" data-id="${g.id}" data-name="${g.name}" style="cursor: pointer; text-align: center; padding: 20px;">
                    <div class="thumbnail" style="border-radius: 50%; overflow: hidden; width: 100px; height: 100px; margin: 0 auto;">
                        <img src="${iconUrl}" alt="${g.name}" style="width: 100%; height: 100%; object-fit: cover;">
                    </div>
                    <div class="track-info" style="margin-top: 15px;">
                        <p class="title" style="font-size: 16px; font-weight: bold;">${g.name}</p>
                    </div>
                </div>`;
            });
            $("#admin-server-grid").html(gridHtml);
        } else {
            $("#admin-server-grid").html("<p style='color: var(--text-muted);'>No servers found where you have Administrator permissions.</p>");
        }
    ...
    getVoiceChannels: function (player, data) {
        if (!data.channels || data.channels.length === 0) {
            $("#setting-voice-channels").html('<option value="">No Voice Channels Found</option>');
            return;
        }
        let html = '<option value="">Select Voice Channel...</option>';
        data.channels.forEach(c => {
            html += `<option value="${c.id}">${c.name}</option>`;
        });
        $("#setting-voice-channels").html(html);
    },
