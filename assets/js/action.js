...
$(document).ready(function () {
        ...
        if ($modal.length) {
            ...
            if ($target.hasClass("submit")) {
                ...
                const result = $modal
                    .find('[data-id^="field-"]')
                    .map(function () {
                        const key = $(this).attr("data-id").replace("field-", "")
    ...
    function buildExplorePage() {
        ...
        if ($("#viral-top-tracks").has(".skeleton").length) {
            player.send({
                op: "getTracks",
                query: "https://music.youtube.com/playlist?list=PL4fGSI1pDJn69On1f-8NAvX_CYlx7QyZc",
                callback: "viral-top-tracks",
            })
        }

        
        if ($("#new-top-tracks").has(".skeleton").length) {
            player.send({
                op: "getTracks",
                query: "https://music.youtube.com/playlist?list=PL4fGSI1pDJn65z9q8iOqZ-s3X57iTzR-r",
                callback: "new-top-tracks",
            })
        }
    ...
    $(document).on("click", "#like-btn", function () {
        ...
        const isLiked = player.playlists?.["200"]?.["tracks"]?.includes(player.currentTrack.trackId);
        if (isLiked) {
        ...
        player.send({
            op: "updatePlaylist",
            type: isLiked ? "removeTrack" : "addTrack",
            playlistId: "200",
            tracks: [player.currentTrack.trackId]
        });
    });

    $("#menu-favourite-page").click(function () {
        const playlistId = "200";
        const playlist = player.playlists[playlistId];
        if (!playlist) return;
        const pageId = `playlist-page-${playlistId}`;
        
        if ($(`#${pageId}`).length) {
            $(`#${pageId}`).replaceWith(buildPlaylistHtml(playlistId, playlist, "user-playlist"));
        } else {
            $(".main-container").append(buildPlaylistHtml(playlistId, playlist, "user-playlist"));
        }
        changePage(pageId, true, false);
...
$(document).ready(function() {
    $(document).on("change", "#setting-autoplay", function() {
        if (!player || !window.selectedAdminGuild) return;
        player.send({ op: "toggleAutoplay", guildId: window.selectedAdminGuild, status: this.checked });
    });
    $(document).on("change", "#setting-247", function() {
        if (!player || !window.selectedAdminGuild) return;
        player.send({ op: "toggle247", guildId: window.selectedAdminGuild, status: this.checked });
    });
    $(document).on("change", "#setting-dj", function() {
        if (!player || !window.selectedAdminGuild) return;
        player.send({ op: "toggleDj", guildId: window.selectedAdminGuild, status: this.checked });
    });

    $(document).on("click", "#setting-join", function() {
        if (!player || !window.selectedAdminGuild) return;
        const channelId = $("#setting-voice-channels").val();
        if (!channelId) return alert("Please select a voice channel first!");
        player.send({ op: "joinVoice", guildId: window.selectedAdminGuild, channelId: channelId });
    });

    $(document).on("click", ".admin-server-card", function(e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        window.selectedAdminGuild = $(this).attr("data-id");
        const name = $(this).attr("data-name");
        
        // Force immediate display bypass fadeOut just in case it's broken
        $("#server-list-view").hide();
        $("#server-config-view").show();
        
        $("#server-config-title").text(name + " Settings");
        $("#setting-voice-channels").html('<option value="">Loading Channels...</option>');
        if (player) {
            player.send({ op: "getVoiceChannels", guildId: window.selectedAdminGuild });
        }
    });

    $(document).on("click", "#back-to-server-list", function(e) {
        e.preventDefault();
        e.stopPropagation();
        window.selectedAdminGuild = null;
        $("#server-config-view").fadeOut(200, () => {
            $("#server-list-view").fadeIn(200);
        });
    });
});
